...
export default async function DocumentAutomationPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    ...
    <SeoSection title="Belgelenmiş belge işleme ve muhasebe vakası" muted><div className="rounded-3xl border border-outline-variant bg-surface p-7 md:p-10"><p className="mb-4 text-sm font-semibold uppercase tracking-wider text-primary-fixed">Anonimleştirilmiş gerçek otomasyon vakası</p><h3 className="mb-5 text-2xl font-bold text-on-surface">Belge İşleme ve Muhasebe Otomasyonu</h3><div className="grid gap-8 md:grid-cols-2"><div className="space-y-4 leading-relaxed text-on-surface-variant"><p>Başlangıçta muhasebe ve birincil belgelerin kaydı ile işlenmesi önemli çalışan eforu gerektiriyor ve manuel veri girişi hatası olasılığı oluşturuyordu.</p><p>Otomatik akış belge taraması veya fotoğrafını; e-posta ya da paylaşılan kaynaklardan alabiliyor, bilgiyi tanıyor, ilgili alanları çıkarıyor, doğruluyor ve muhasebe sistemi verisiyle karşılaştırıyordu. Gerekli matematiksel kontrollerin ardından belge kaydediliyor ve muhasebe sisteminde ilgili bilgi oluşturuluyordu.</p></div><div className="grid gap-4 sm:grid-cols-2 md:grid-cols-1"><div className="rounded-2xl bg-ink p-6"><p className="font-headline text-4xl font-extrabold text-white">3–20x</p><p className="mt-2 text-white/70">Belge kayıt işinde daha düşük iş gücü gereksinimi</p></div><div className="rounded-2xl bg-ink p-6"><p className="font-headline text-4xl font-extrabold text-white">2–3x</p><p className="mt-2 text-white/70">Gelen belge işleme hızında artış</p></div></div></div><p className="mt-7 text-sm text-on-surface-variant">Bu sonuçlar yalnızca belgelenmiş vaka çalışmasına aittir. Her uygulamanın sonucu belge yapısına, hacme, sistemlere ve kontrol gereksinimlerine göre değişebilir.</p></div></SeoSection>
