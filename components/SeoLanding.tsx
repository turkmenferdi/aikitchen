    ...
    <section className="relative overflow-hidden border-b border-outline-variant/60 bg-white">
      ...
      <div className="relative mx-auto max-w-5xl px-4 pb-16 pt-8 sm:px-6 md:pb-20 lg:px-8">
        <nav aria-label="Breadcrumb" className="mb-10 flex items-center gap-1.5 text-sm text-on-surface-variant">
          ...
          <Link href="/tr/platform" className="hover:text-primary">
            Süreç Otomasyonu
          </Link>
          <ChevronRight size={14} />
          <span className="text-on-surface">{eyebrow}</span>
        </nav>
        <p className="mb-4 text-sm font-semibold uppercase tracking-[0.14em] text-primary">{eyebrow}</p>
...
export function SeoSection({ title, intro, children, muted = false }: { title: string; intro?: string; children: ReactNode; muted?: boolean }) {
  return (
    <section className={`${muted ? 'border-y border-outline-variant/60 bg-white' : 'bg-surface'} py-16 md:py-20`}>
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {items.map(([title, description]) => (
        <article key={title} className="flex gap-4 rounded-2xl border border-outline-variant bg-white p-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      {links.map(([label, href]) => (
        <Link
          key={href}
          href={href}
          className="group flex items-center justify-between rounded-2xl border border-outline-variant bg-surface p-5 font-semibold text-on-surface transition-colors hover:border-primary/40 hover:bg-white"
        >
          <span>{label}</span>
